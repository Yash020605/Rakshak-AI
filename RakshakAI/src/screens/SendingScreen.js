...
export default function SendingScreen({ route, navigation }) {
  ...
  const sendSOS = async () => {
    ...
    // Hard cap — navigate to Map after 8s using whatever location we have
    ...
    try {
      ...
      setTimeout(() => {
        ...
        setNearestService(result.nearestService);
        setTimeout(() => {
          navigation.replace('Map', {
            type,
      ...
      clearTimeout(hardTimeout);
      setSteps(s => ({ ...s, location: !!capturedLocation, contacts: false, service: false }));
      // Navigate to map with real location if we have it, otherwise show error
      setTimeout(() => {
        if (capturedLocation) {
          navigation.replace('Map', {
            type,
          ...
          navigation.replace('Map', { type, latitude: null, longitude: null, alertId: null, nearestService: null });
        }
      }, 2000);
    }
  };
  ...
  return (
    <View style={[styles.container, { backgroundColor: color }]}>
      ...
      <View style={styles.circleWrap}>
        <Animated.View style={[styles.outerCircle, { transform: [{ scale: pulseAnim }], borderColor: 'rgba(255,255,255,0.3)' }]} />
...
const styles = StyleSheet.create({
  ...
  outerCircle: { position: 'absolute', width: 200, height: 200, borderRadius: 100, borderWidth: 3 },
  ...
  doNotClose: { position: 'absolute', bottom: 50, backgroundColor: 'rgba(0,0,0,0.2)', paddingHorizontal: 24, paddingVertical: 12, borderRadius: 30 },
